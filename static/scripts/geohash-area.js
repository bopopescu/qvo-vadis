...
function geohash_area_distance(start,end) {  // THIS FUNCTION IS NOT USED, IS IT?
    /**
     * @param {Object}  start   coordinates with lat and lng properties
     * @param {Object}  end     coordinates with lat and lng properties
     *
     * @return {number}         distance between the two points in ??
     */
...
function geohash_area_area(fn,precision) {
    /**
     * @param {function}  fn    function that recursively estimates whether a supplied geohash touches the area of
     *                          interest. This function should return 2 if the particular geohash is completely within
     *                          the area boundaries being tesed.
     * @param {number}    precision     required geohash precision
     *
     * @return {number}   array of geohash coordinates of the required precision that cover the area intrinsic to
     *                    fn()
     */
  var res = [];
...
function geohash_area_rect(lat,lng,dist,precision) {
  ...
  return geohash_area_area(checkRect(box),precision);
}

function geohash_area_encode(lat, lon, precision) {  // THIS FUNCTION IS NOT USED, IS IT?
...
function geohash_area_decode(hash) {  // THIS FUNCTION IS NOT USED, IS IT?
  var bounds = {
    bottom: -90.0,
    top: 90.0,
    left: -180,
